  ...
  // Model to edit appointment status 
   $('#EDITRDVModal').on('show.bs.modal', function (event) {
    ...
    $('#ArchiveModal').on('show.bs.modal', function (event) {
      ...
      var modal = $(this)
      modal.find('#archive_link').attr("href", link)
    })
          ...
          function AddAppointment(date){
            ...
            $.ajax({
                ...
                success: function(array){ 
                      ...
                      if($('#sms').is(":checked")){
                          modal.find('#notifyby_sms').val(1)
                      }else{
                          modal.find('#notifyby_sms').val(0)
                      }
                      if($('#whatsapp').is(":checked")){
                          modal.find('#notifyby_whatsapp').val(1)
                      }else{
                          modal.find('#notifyby_whatsapp').val(0)
                      }
                      if($('#email').is(":checked")){
                          modal.find('#notifyby_email').val(1)
                      }else{
                          modal.find('#notifyby_email').val(0)
                      }
                    })
